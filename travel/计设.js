...
document.addEventListener('DOMContentLoaded', function() {
    ...
    postImages.addEventListener('change', function(e) {
        ...
        if (uploadedImages.length + files.length > 9) {
            showToast('warning', '图片数量超限', '最多只能上传9张照片');
            return;
        }

        files.forEach(file => {
            if (!file.type.startsWith('image/')) {
                showToast('warning', '文件类型错误', '请上传图片文件');
                return;
            }
            ...
            reader.onload = function(e) {
                ...
                addImagePreview(e.target.result, uploadedImages.length - 1);
            };
    ...
    postTags.addEventListener('keypress', function(e) {
        if (e.key === 'Enter') {
            ...
            if (tag && !postTagsList.includes(tag)) {
                if (postTagsList.length >= 5) {
                    showToast('warning', '标签数量超限', '最多只能添加5个标签');
                    return;
                }
                postTagsList.push(tag);
                renderTags();
            }
            this.value = '';
        }
    ...
    function renderTags() {
        tagsContainer.innerHTML = '';
        postTagsList.forEach((tag, index) => {
    ...
    function submitPost() {
        ...
        if (!title) {
            showToast('warning', '信息不完整', '请填写帖子标题');
            return;
        }

        if (!content) {
            showToast('warning', '信息不完整', '请填写旅游故事内容');
            return;
        }

        if (content.length > 1000) {
            showToast('warning', '内容过长', '故事内容不能超过1000字');
            return;
        }
        ...
        .then(data => {
            if (data.success && data.post) {
                    ...
                    uploadedImages.forEach(img => {
                        imagesHtml += `
                            <div class="img-placeholder">
                                <img src="${img}" alt="旅行照片" style="width: 100%; height: 100%; object-fit: cover;">
                            </div>
                        `;
                    });
                    imagesHtml += '</div>';
                ...
                closePostModal();
                showToast('success', '发布成功', '您的旅游故事已发布');
            } else {
    ...
    if (communitySearchBtn && communitySearchInput) {
    ...
    function searchCommunityPosts() {
        ...
        // 模拟搜索功能
        showToast('info', '搜索中', `正在搜索包含"${query}"的帖子`);
        ...
        setTimeout(() => {
            ...
            posts.forEach(post => {
                ...
                const postUser = post.querySelector('.user-info h4')?.textContent || '';
                const postLocation = post.querySelector('.user-info span')?.textContent || '';
                const postTags = Array.from(post.querySelectorAll('.post-tag')).map(tag => tag.textContent).join(' ');

                if (postTitle.includes(query) || postContent.includes(query) || postUser.includes(query) || postLocation.includes(query) || postTags.includes(query)) {
                    found = true;
            ...
            if (found && firstMatch) {
                // 滚动到第一个匹配的帖子
                firstMatch.scrollIntoView({ behavior: 'smooth', block: 'center' });
                showToast('success', '搜索成功', `找到与"${query}"相关的帖子`);
            } else {
                showToast('info', '搜索结果', `未找到与"${query}"相关的帖子`);
            }
        }, 500);
    }
